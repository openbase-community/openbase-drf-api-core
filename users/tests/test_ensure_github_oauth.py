...
def test_ensure_github_oauth_skips_without_credentials_in_non_interactive_mode(
    ...
    call_command("ensure_github_oauth", non_interactive=True, stdout=stdout)
...
def test_ensure_github_oauth_uses_credentials_json_for_default_site():
    call_command("ensure_default_sites")
    credentials_json = """
    {
      "client_id": "client-id",
    ...
    call_command(
        "ensure_github_oauth", credentials_json=credentials_json, name="Dev App"
    )
...
def test_ensure_github_oauth_uses_requested_site_domain():
    ...
    call_command(
        ...
        name="Wellness Navigator GitHub",
        site_domains=["api.mindfulmakers.xyz"],
    )

    social_app = SocialApp.objects.get(provider="github")

    assert social_app.name == "Wellness Navigator GitHub"
    assert list(social_app.sites.values_list("id", flat=True)) == [site.id]


def test_ensure_github_oauth_reuses_matching_credentials_for_new_site():
    call_command("ensure_default_sites")
    ...
    assert SocialApp.objects.filter(provider="github").count() == 1
    social_app = SocialApp.objects.get(provider="github")
    assert set(social_app.sites.values_list("id", flat=True)) == {1, other_site.id}


def test_ensure_github_oauth_requires_requested_site_domain_to_exist():
    credentials_json = """
    {
      "client_id": "client-id",
      "client_secret": "client-secret"
